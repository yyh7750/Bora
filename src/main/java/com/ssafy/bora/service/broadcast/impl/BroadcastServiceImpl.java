...
public class BroadcastServiceImpl implements IBroadcastService {
    ...
    public List<BasicMainDTO> findTopTenBroadcast() {
        ...
        for (TopTenDTO ttDto : list) {
            Optional<Station> station = stationRepository.findStationByDjId(ttDto.getUserId());
            if (station.isPresent()) {
                BasicMainDTO bmDTO = BasicMainDTO.convertEntityToBasicMainDTO(station.get(), ttDto);
    ...
    public List<MyFollowBroadcastDTO> findFollowBroadcast(String id) {
        ...
        for (Follow follow : list) {
            //내가 팔로우하는 방송국
            Optional<Station> station = stationRepository.findStationByDjId(follow.getDj().getId());
            //방송중인 유저
            Broadcast broadcast = broadcastRepository.findByUserAndEndBroadIsNull(follow.getDj());
            boolean isLive = broadcast != null;
            MyFollowBroadcastDTO mfbDTO = MyFollowBroadcastDTO.convertEntityToMyFollowBroadcastDTO(isLive, station.get());
